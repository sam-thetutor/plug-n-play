...
import { PlugTransport } from "@slide-computer/signer-transport-plug"; // Import PlugTransport from signer-transport-plug
...
function fromBase64(base64Str: string): ArrayBuffer {
...
const dappOrigin = () => {
  return get(page).url.hostname.startsWith("localhost")
  ? "http://localhost:5173"
  : "https://kongswap.io";
} 
...
const STORAGE_KEY = 'kong-delegation';
...
const delegationStorage = new LocalDelegationStorage();

// Helper functions for delegation chain management
async function getDelegationChain(key: string, storage: DelegationStorage) {
  const data = await storage.get(`${key}-delegation`);
  if (!data) return null;
  const chainData = JSON.parse(data);
  return DelegationChain.fromDelegations(
    chainData.delegations.map((d: any) => ({
      delegation: new Delegation(
        fromBase64(d.delegation.pubkey),
        BigInt(d.delegation.expiration),
        d.delegation.targets?.map((p: string) => Principal.fromText(p))
      ),
      signature: fromBase64(d.signature)
    })),
    fromBase64(chainData.publicKey)
  );
}

async function setDelegationChain(key: string, chain: DelegationChain, storage: DelegationStorage) {
  const chainData = {
    delegations: chain.delegations.map(d => ({
      delegation: {
        pubkey: toBase64(d.delegation.pubkey),
        expiration: d.delegation.expiration.toString(),
        targets: d.delegation.targets?.map(t => t.toString())
      },
      signature: toBase64(d.signature)
    })),
    publicKey: toBase64(chain.publicKey)
  };
  await storage.set(`${key}-delegation`, JSON.stringify(chainData));
}

async function removeDelegationChain(key: string, storage: DelegationStorage) {
  await storage.remove(`${key}-delegation`);
}
...
export const walletStore = writable<{
  agent: HttpAgent |SignerAgent<any> | null,
...
export async function connectWithNFID(wallet: 'plug' | 'nfid') {
  ...
  try {
    ...
    const delegationChain = await signer.delegation({
      ...
      targets: [Principal.fromText(kongBackendCanisterId)],
...
export async function connectWithPlug() {
  try {
    walletStore.update(store => ({ ...store, isConnecting: true, error: null }));

    // Create Plug transport
    const transport = new PlugTransport();

    // Create signer with Plug transport
    const signer = new Signer({
      transport
    });


    // Get accounts to get the principal
    const accounts = await signer.accounts();
   ...
   const agent = HttpAgent.createSync({ 
    ...
    // Create signer agent with the first account
    ...
    // Store the active transport for cleanup during disconnect
    activeTransport = transport;

    // Update wallet store with new signer agent
    ...
    localStorage.setItem("selectedWalletId", "plug");
    selectedWalletId.set("plug");
...
const createDelegationPermissionScope = () => {
  return {
    ...
    targets: [Principal.fromText(kongBackendCanisterId), Principal.fromText(kongFaucetCanisterId)] // optional array of Principal targets
  };
};
...
// Track agents
let anonymousAgent: HttpAgent | SignerAgent<any> | null = null;

export async function getAnonymousAgent(): Promise<HttpAgent | SignerAgent<any>> {
  if (!anonymousAgent) {
    anonymousAgent = HttpAgent.createSync({
      host: HOST
    });
...
export async function disconnectWallet() {
  try {
    // Remove delegation chain from storage
    await removeDelegationChain(STORAGE_KEY, delegationStorage);

    // Disconnect from wallet
    if (get(walletStore).agent) {
      walletStore.update(store => ({
        ...store,
        agent: null,
        signerAgent: null,
